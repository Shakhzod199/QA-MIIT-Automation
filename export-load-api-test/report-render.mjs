// Pure HTML-rendering helpers — no k6 or Node-specific APIs, no external
// imports — so this file loads identically whether it's bundled into a k6
// test script or `import`ed by a plain Node script (build-report-index.mjs)
// assembling the combined report after the run finishes.
...
// Everything *except* `body` — standalone pages and the merged multi-section
// page each need their own body rules (padding/header differ), but share
// every other rule so the two never visually drift apart.
export const SHARED_STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .summary.ok { border: 1px solid rgba(16,185,129,0.3); background: rgba(16,185,129,0.08); }
  .summary.fail { border: 1px solid rgba(239,68,68,0.3); background: rgba(239,68,68,0.08); }
  .summary .icon { width: 20px; height: 20px; flex-shrink: 0; }
  .summary.ok .icon { color: #34d399; }
  .summary.fail .icon { color: #f87171; }
...
/**
 * Renders the report's inner markup (title, pass/fail summary, and the
 * Thresholds/Checks/HTTP/Other sections) with no surrounding <html>/<body> —
 * callers wrap it themselves, either standalone (wrapHtmlDocument below) or
 * merged alongside another report's body in the same page.
 */
export function renderReportBody(data, title) {
  ...
  <div class="summary ${allThresholdsOk ? "ok" : "fail"}">
...
/** Wraps a rendered body as a standalone, self-contained HTML document. */
...
/** Standalone report (used for the individual load-report.html / stress-report.html artifacts). */
export function buildHtmlReport(data, title) {
  return wrapHtmlDocument(title, renderReportBody(data, title));
}
